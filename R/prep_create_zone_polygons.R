...
get_buffer_conditional <- function(buffer_distance, anps_proj, area_fld='buff_area_ha'){
  # NPA buffer 
  buffer_fp <- str_c('data/intermediate_data/anps_buffer_', buffer_distance,'km.gpkg')
...
# Mexico from GADM
mex_gadm <- raster::getData('GADM', country='MEX', level=0, 
                            path='data/input_data/context_Mexico') %>% 
  st_as_sf(crs=4326) %>% 
  st_transform(crs=crs) %>% 
  st_simplify(dTolerance=40, preserveTopology=T)
...
# Voronoi polygons
# vpols <- make_voronois(anps_buffin, mex_gadm)
vpols <- get_voronois_conditional(anps_terr, buffer_distance, region_poly=mex_gadm) %>% 
  st_transform(crs=crs)
